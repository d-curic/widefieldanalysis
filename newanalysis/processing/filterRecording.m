...
function filteredIntensities =  filterRecording(pixelIntensity, exp)
tic;
%for fn=1:4
nFrames = exp.numframes;
...
% pathname = 'C:\Users\surjeet.singh\Desktop\20170314\';
% file={'02_spon_90000fr_100Hz_raw.raw','08_spon_90000fr_100Hz_raw.raw','09_spon_90000fr_100Hz_raw.raw','10_spon_90000fr_100Hz_raw.raw'};
...
% %change path IMPORTANT
% Mask = imread('C:\Users\surjeet.singh\Desktop\20170314\Mask.tif','tiff');
...
% end    
rows = size(exp.mask,1);
columns = size(exp.mask,2);
trim = 0;
%rows*columns
NonZeroPixelsIndex = 1:size(pixelIntensity(:,1));%find(ones(1,rows*columns));
...
'FT filtering ... '
filter_coefficients = load(exp.coefffile);
...
% tic
filteredIntensities = reshape( dXrecon_dff0_filtered',rows,columns,size(dXrecon_dff0_filtered,1)).*single(exp.mask);
